...
#define LAN9252_SPI_HANDLE hspi3
...
void mem_test(void)
{
	...
	//如果有错误 建议排查硬件SPI总线以及XML文件  尤其EEPROM配置  HLABS
	//注意有时候有错误可能你也能读取到正确的测试字节
  //小伙伴们 测试字节和下面的ID  一定要保证正确。尤其简单XML文件的配置 当前还没有STM32的SSC模板 很容易忘记！ HLABS
	//有时间自己搞一份SSC的STM32模板文件  HLABS
	if(temp != 0x87654321)
	{
		
		while(1)
		{
			printf("stop,spi test_read is error;\n");
			HAL_Delay(500);
		}
	}
	...
	if(temp != 0x9252)//HLABS  如果您更换为其它型号 记得 屏蔽这里或者修改判断
	{
		while(1)
		{
				printf("!! ID is not 9252 !!\n");
			  HAL_Delay(500);
		}
	}
}
...
void SPIWrite(UINT8 data)
{
	  ...
	  HAL_SPI_Transmit(&LAN9252_SPI_HANDLE,&data,1,2000);
}
UINT8 SPIRead()
{
		UINT8 data;
    HAL_SPI_Receive(&LAN9252_SPI_HANDLE,&data,1,2000);
    return (data);
